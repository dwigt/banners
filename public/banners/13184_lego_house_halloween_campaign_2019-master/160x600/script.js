...
function politeInit() {
    ...
    $(function() {
        ...
        var banner_elements = [
            spiderbro, bg, dracula1, bat1, bat2, pumpkins
        ];
        ...
        function animate_banner() {
            ...
            .from([bat2], .75, { left: "-100%", ease: Power4.easeOut }, 0.25, "start+=1.5")
                .from([bat1], .75, { left: "100%", ease: Power4.easeOut }, 0.25, "start+=2.25")
                //4 seconds
                .add(hoverBatLeft(bat1), "start+=2.25")
                .add(hoverBatRight(bat2), "start+=2.5")
                // .add(slideVertical(dracula1, "top"), "start+=3")
            if (loops < max_loops) {
                mytl
